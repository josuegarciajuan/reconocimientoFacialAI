"""Tests de superres.py: fallback LANCZOS4, salvaguardas, no-rotura sin SR y frame_face."""
import numpy as np
...
def test_zoom_photo_native_sr_size():
    ...
    assert out.dtype == np.uint8
    # crop ~50px -> fallback a sr_target_side (512); con SR real seria ~200px
    assert max(out.shape[:2]) >= 1
    assert out.ndim == 3 and out.shape[2] == 3
