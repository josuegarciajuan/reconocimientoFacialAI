...
|---|---|---|---|---|
| B9 | SQL injection global (sin escape ni prepared statements) | `libs/mysql.class.php` + todos | Crítica | 4 | 🔶 casi completo: mutaciones + listados migrados; queda barrido final read-only (camaras/locales/config/visitantes edit + notificaciones + seleccionCuenta + index) |
...
| B16 | Índices de líneas desalineados al guardar | `config/javascript.php` | Media | 4 | ⏳ pendiente (JS) |
| B17 | Asignaciones a `$aux` en vez de `$data[$i]`; "Descargar" sin handler | `fichajes/*` | Media | 4 | ✅ agregación reescrita en PDO |
| B18 | `subir_video2` no dispara el procesamiento | `visitantes/acciones.php` | Media | 4 | ⏳ 4c (enrolamiento.py) |
| B19 | `?descargar=` roto (`$tmp` sin instanciar) | `admin/index.php` | Baja | 4 | ⏳ pendiente |
| B20 | `exit;` hardcodeado (depuración) | `capturador.php` | Alta | 4 | ⏳ pendiente |
