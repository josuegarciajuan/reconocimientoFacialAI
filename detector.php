...
while (true) {
    ...
    $sql->Consultar("locales", "id", "id>0", "id asc", false);
    if ($sql->num > 0) {
        do {
            $local_id = (int)$sql->row["id"];

            $tmp->Consultar("camaras", "id", "local_id=" . $local_id . " and encendida=1", "id asc", false);
            if ($tmp->num > 0) {
                do {
                    $cam_id = (int)$tmp->row["id"];
